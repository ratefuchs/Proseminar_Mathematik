...
Da $\cos\varphi_n$ und $\sin\varphi_n$ nicht von $r$ oder $\varphi_k \quad (k=1,\ldots,n-1)$ abhängt und $P_n(r,\varphi_1,\ldots,\varphi_{n-1})$ nicht von $\varphi_n$, gilt:\\[1.5em]
$ \partial_r P_{n+1}(r,\varphi_1,\ldots,\varphi_n)= \partial_r (P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n) + \partial_r (r\sin\varphi_n e_{n+1})= \partial_r P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n + \sin\varphi_n e_{n+1} = \begin{pmatrix}\partial_r P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n \\ \sin\varphi_n \end{pmatrix} $\\[1.5em]
$ \partial_{\varphi_k}P_{n+1}(r,\varphi_1,\ldots,\varphi_n) = \partial_{\varphi_k} (P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n) + \partial_{\varphi_k} (r\sin\varphi_n e_{n+1})= \partial_{\varphi_k} P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n = \begin{pmatrix}\partial_{\varphi_k} P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n \\ 0 \end{pmatrix} \qquad (k=1,\ldots,n-1)$\\[1.5em]
$ \partial_{\varphi_n}P_{n+1}(r,\varphi_1,\ldots,\varphi_n)=\partial_{\varphi_n}(P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\cos\varphi_n) + \partial_{\varphi_n} (r\sin\varphi_n e_{n+1})=-(P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\sin\varphi_n+r\cos\varphi_n e_{n+1} = \begin{pmatrix}-P_n(r,\varphi_1,\ldots,\varphi_{n-1})\cdot\sin\varphi_n \\ r\cos\varphi_n \end{pmatrix} $\\[1.5em]
Behauptung:
\begin{equation}
\label{det}\det(J_{P_{n+1}}(r,\varphi_1,\ldots,\varphi_n))=\det(J_{P_n}(r,\varphi_1,\ldots,\varphi_{n-1}))\cdot r\cos^{n-1}\varphi_n
\end{equation}
Denn für $\cos\varphi_n=0$ ist wegen $\partial_{\varphi_1}P_{n+1}(r,\varphi_1,\ldots,\varphi_n)=0$ schon $\det(J_{P_{n+1}}(r,\varphi_1,\ldots,\varphi_n))=0$.\\
...
$\kappa_n=2\pi\frac{R^n}{n}\prod\limits_{k=2}^{n-1}\left(\int\limits_{-\frac{\pi}{2}}^\frac{\pi}{2}\cos^{k-1}\varphi_k d\varpi_k\right)=2\pi\frac{R^n}{n}\cdot\sqrt{\pi}^{n-2}\cdot\frac{\Gamma(1)}{\Gamma(\frac{3}{2})}\cdot\frac{\Gamma(\frac{3}{2})}{\Gamma(2)}\cdot\ldots\cdot\frac{\Gamma(\frac{n-1}{2})}{\Gamma(\frac{n}{2})}
...
\subsection{Sonstiges}%TODO:evtl. auch rausnehmen
...
\subsection{\glqq Ellipsoidkoordinaten\grqq}%Kommentar: waren bei einer Aufgabe auf einem HM2-Übungsblatt tatsächlich hilfreich...
